...
<head>
    ...
    */
   	 ...
   	 */
   	$dbh = new DB('_s0570024__mockupdatadb', 'db.f4.htw-berlin.de:3306', 's0570024', $pass);
   ...
   function debug_to_console( $data ) {
       ...
       if ( is_array( $data ) )
           $output = "<script>console.log( 'Debug Objects: " . implode( ',', $data) . "' );</script>"; //das ist Javascript
       else
...
<body>
    ...
    //debug_to_console("start");
	   if ($_GET)
	   {
        ...
        elseif($command=='edit')
        {
            $teilnehmerin=$dbh->get($id);   //Wir holen uns den Datensatz für die diese ID, speichern sie in teilnehmerin und können jetzt die Array-Inhalte auslesen.
            /*print_r($teilnehmerin);   print_r gibt ein Array aus. Einfach mal zum Testen, was die get-Methode gespeichert hat.
	   ...
	   //debug_to_console("in POST");
             ...
             * in die Datenbank
             */
            $vorname=filter_var($_POST['vorname'], FILTER_SANITIZE_STRING); //damit hier nicht ein Schadcode, JVscript, eingefügt werden kann
           $nachname=filter_var($_POST['nachname'], FILTER_SANITIZE_STRING);
           $email=filter_var($_POST['email'], FILTER_SANITIZE_EMAIL);
           $ipnr=filter_var($_POST['ipnr'], FILTER_SANITIZE_STRING);

           if (isset($_POST['id']))                       // Wenn das POST-Array auch eine id erhält, dann wird ein bestehender Datensatz geändert
           {
               $id=filter_var($_POST['id'], FILTER_SANITIZE_NUMBER_INT);
               $dbh->edit(array($vorname, $nachname, $email, $ipnr, $id));
           }

           else                                  //Wenn wir einen neuen Datensatz anlegen
               {
               $dbh->add($vorname, $nachname, $email, $ipnr);
           }
   ...
   <div class="container">
      <div class="panel panel-default">
         ...
         <div class="panel-body">
            ...
            <form role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST"> <!--ruft die Seite noch mal auf. Ginge auch: action="aufgabe7.php"-->
             <!--
             ...
             -->
                ...
                <input type="text" class="form-control" name="vorname" value="<?php echo $teilnehmerin['vorname'];?>"/> <!--name= wird als Schlüssel gebraucht, um auf die Werte in der DB zuzugreifen-->
                ...
                <!--jetzt ist unser Post-Array befüllt, weil wir bei form oben die Methode Post gewählt haben-->
            ...
            if (!sizeof($teilnehmerinnen)) {
               ...
               // $teilnehmerin = $dbh->get(6);
               {
                  ...
                  '<div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
                     <div class="thumbnail">  <!--im CSS: Wenn man über diese Klasse hovert, wird die Klasse buttons-edit sichtbar -->
